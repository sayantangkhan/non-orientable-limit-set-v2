...
We begin by finding Teichmüller geodesic segments whose endpoints lie in $\systole$ such that at some point in the interior, some one-sided curve gets very short.
...
Observe that by making the length of $c$ (and correspondingly $c^{\prime}$, $d$, and $d^{\prime}$) go to $0$, while keeping the area $1$ lets us embed an annulus of high modulus around any curve in $\{c, c^{\prime}, d, d^{\prime}\}$.
This shows that the DQD we constructed satisfies condition (a).

Checking condition (b) is easy, but tedious.
For convenience, we have labelled the core curves of the vertical cylinders: the reader can check that they are all two-sided, and deleting them results in orientable subsurfaces.
...
\end{enumerate}
By making $c$ smaller, while keeping the area equal to $1$, one can embed an annulus of high modulus in the DQD, as seen in \autoref{fig:genus-9-example}.
This shows that our construction satisfies condition (a).

To see that deleting the core curves of the horizontal cylinders results in orientable subsurfaces, note that deleting the labelled subset of the core curves results in $2$ pairs of pants, and a genus $3$ orientable surface with one boundary component.
