...
\end{question}
We know from \autoref{thm:rational-approximation} that the answer to the above question is yes for $\Gamma = \mcg(\no_g)$ (for $g \geq 3$).
...
\end{lemma}
\begin{proof}
  Let $M_L$ denote the set of all measures $\mu$ such that $d_{\mathrm{LP}}(\mu, T\mu) \leq \frac{2}{L}$.
  ...
  By compactness, we have that for some large enough $L$, $M_L$ must be in a $\varepsilon$-neighbourhood of the set of invariant measures, and every orbit measure of length greater than $L$ must be in this $M_L$.
\end{proof}
...
\begin{proof}[Proof of \autoref{thm:rational-approximation}]
  ...
  Denote the geodesic tightening by $c_i^{\prime}$: we have that its intersection number with $\lambda_j$ is equal to the Euclidean distance between $p_0$ and $p_i$ which goes to $0$ as $i$ goes to $\infty$.
