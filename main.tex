...
We formally state this as a lemma, and sketch a proof.
\begin{lemma}
...
\begin{proof}[Sketch of proof]
  ...
  We denote the constant speed parameterization of $\widehat{\gamma_i}$ and $\widehat{\gamma_{i}^{\prime}}$ by the same symbols as well: recall that a these are constant speed maps from $[0,1]$ to $\mathbb{H}^2$.
  Pick any small $\varepsilon \in (0, 1)$, and consider the parameterized geodesic arcs restricted to $(\varepsilon, 1- \varepsilon)$. For large enough $i$, if $\widehat{\gamma_i(x)}$ is also in $\widehat{\delta}$, then for some other point $x_i^{\prime}$ in the neighbourhood of $x$, $\widehat{\gamma_i^{\prime}(x^{\prime})}$ is also in $\widehat{\delta}$ and vice versa (checking this fact requires some elementary hyperbolic geometry which we skip).
  Since the lengths of $\widehat{\gamma_i}$ and $\widehat{\gamma_i^{\prime}}$ go to $\infty$, and we can pick $\varepsilon$ to be arbitrarily small, we have convergence to the same foliation in $\mf(S)$.
\end{proof}
...
\begin{proposition}
  ...
  Let $\gamma_1$ and $\gamma_2$ be two measured foliations on a compact surface $S$ satisfying the following conditions.
  \begin{itemize}
  ...
  \item There exist a sequence of curves converging to $\gamma_1$ and $\gamma_2$ in $\pmf(S)$.
  % \item $\gamma_1$ and $\gamma_2$ are approximable by simple two-sided multicurves.
  \end{itemize}
  Then for any $c \in (0, \infty)$, there exists a sequence of curves converging to $c \gamma_1 + \gamma_2$ in $\mf(S)$.
\end{proposition}

  % \todo[inline]{Make proof clearer by emphasizing on the importance of the multiplicity $m_j$.}
\begin{proof}
  Let $\{\gamma_{1j}\}$ and $\{ \gamma_{2j}\}$ be sequences of simple two-sided curves converging to $\gamma_1$ and $\gamma_2$ in $\pmf(S)$.
  Since the intersection number of $\gamma_1$ and $\gamma_2$ with the Liouville current is $1$, we have that $\frac{\gamma_{1j}}{\lhyp(\gamma_{1j})}$ approaches $\gamma_1$ in $\mf(S)$, and $\frac{\gamma_{2j}}{\lhyp(\gamma_{2j})}$ approaches $\gamma_2$ in $\mf(S)$.
  ...
  We claim that $\kappa_j$ divided by $m_j \cdot \lhyp(\gamma_{1j})+ \lhyp(\gamma_{2j})$ converges to $c \gamma_1 + \gamma_2$ in $\mf(S)$.
  ...
  Let $\widehat{\kappa}_j$ be a lift of $\kappa_j$ to the universal cover $\mathbb{H}^2$.
  It follows the lifts $\widehat{\gamma}_{1j}$ and $\widehat{\gamma}_{2j}$ of $\gamma_{1j}$ and $\gamma_{2j}$, turning sharply whenever it switches from following a lift of $\gamma_{1j}$ to following a lift of $\gamma_{2j}$ or vice versa.
  Let $\widehat{\kappa}_j^{\prime}$ denote the segments in $\widehat{\kappa}_j$ in the neighbourhood of the turning points where the arc before the turn is within $4 \delta$ distance (where $\delta$ is the Gromov hyperbolicity constant of $\mathbb{H}^2$) of the arc after the turn (see \autoref{fig:turn-label} for an example).
  \begin{figure}[h]
    \centering
    \incfig[0.5]{turn-label}
    % \includegraphics{example-image-a}
    \caption{The segments $\widehat{\kappa}^{\prime}_j$ and $\widehat{\kappa}_j^{\prime \prime}$ in $\widehat{\kappa}_j$.}
    \label{fig:turn-label}
  \end{figure}
  Denote the complement of $\widehat{\kappa}_j^{\prime}$ in $\widehat{\kappa}_j$ by $\widehat{\kappa}_j^{\prime \prime}$.
  We construct a new curve using $\widehat{\kappa}_j^{\prime \prime}$, connecting two successive disconnected pieces by a geodesic arc joining the endpoints (see \autoref{fig:turn-label}).
  We call this new arc in $\mathbb{H}^2$ $\widehat{\lambda}_j$, and its image in the surface $\lambda_j$.
  We claim that $\widehat{\lambda}_j$ is within $4 \delta$ distance of the geodesic tightening of $\widehat{\kappa}_j$.
  This follows from a standard thin triangles argument, so we will elide the details (see \autoref{fig:tightening-arcs}).
  \begin{figure}[h]
    \centering
    \incfig[0.5]{tightening-arcs}
    % \includegraphics{example-image-b}
    \caption{The geodesic tightening stays within a bounded distance of $\widehat{\lambda}_j$.}
  ...
  Consider now the projection of $\widehat{\kappa}_j^{\prime \prime}$ to the surface $S$: we denote the segments of $\gamma_{1j}$ which follow the projection of $\widehat{\kappa}_j^{\prime \prime}$ by $\gamma_{1j}^{\prime \prime}$ and the corresponding segments of $\gamma_{2j}$ by $\gamma_{2j}^{\prime \prime}$.
  ...
  Since the geodesic tightening of $\widehat{\kappa}_j$ is within a bounded distance of $\widehat{\lambda}_j$, for large enough $j$, the intersection number with $\xi$ will be the same, and we will get the following inequality between intersection numbers on the surface $S$.
  \begin{align}
    \label{eq:4}
