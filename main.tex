...
\begin{proposition}
  ...
  For all $g \geq 4$ and any $\varepsilon > 0$, there exists a Teichmüller geodesic segment $l$ whose endpoints lie in $\systole$, and a point $p$ in $l$ such that some one-sided curve has length less than $\varepsilon$ with respect to the hyperbolic metric on $p$.
\end{proposition}

To prove this result, we will need two lemmas relate hyperbolic and flat lengths.
\begin{lemma}
  \label{lem: relating-flat-hyperbolic}
  {\color{red} Lemma about inequalities between flat and hyperbolic lengths.}
\end{lemma}
\begin{proof}[Sketch of proof]
  If $\lhyp(\gamma) \leq \varepsilon$, then there exists an annulus around $\gamma$ of modulus $f(\varepsilon)$, where $f$ is the function that appears in the collar lemma.
  By {\color{red} [Insert ref here]}, this annulus can be homotoped to be a primitive annulus, i.e. an annulus that does not pass through a singularity of the flat metric.
