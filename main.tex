...
The moduli space $\mg(\no_g)$ of compact \emph{non-orientable} hyperbolic surfaces of demigenus $d$ is conjectured to have similarities to infinite volume geometrically finite manifolds (in a manner similar to how moduli spaces of compact orientable surfaces have properties similar to finite volume hyperbolic manifolds).
...
% Given a DQD corresponding a cotangent vector on $\teich(\no_g)$, we define the forward and backward limit points to be the projective classes of the horizontal and vertical foliation of the DQD.
% Note that the Teichmüller geodesic may accumulate on a set larger than its horizontal and vertical foliation (see \cite{2016arXiv160807945L}, \cite{2013arXiv1312.2305L}, \cite{Lenzhen2008}, and \cite{ChaikaMasurWolf}).
% Given two projective measured foliations that collectively fill the surface, i.e. have positive intersection number with every closed curve, one can construct a Teichm\"uller geodesic with the two foliations as forward and backward endpoints.
% This lets us define the notion of the weak convex hull of a subset of $\pmf(\no_g)$ (we borrow this terminology from \cite{farb2002convex}).
% \begin{definition}[Weak convex hull of set of measured foliations]
%   Given a subset $P$ of $\pmf(\no_d)$, the weak convex hull of $P$ is the union of the images of all the Teichm\"uller geodesics whose forward and backward endpoints lie in $P$.
% \end{definition}

% We are interested in understanding what happens to the hyperbolic lengths of certain curves as we move around in the convex hull of $\pmf^+(\no_g)$. Since the description we have of cotangent vectors and convex hulls is
% via flat geometry, rather than hyperbolic geometry, we need a way to estimate hyperbolic lengths of
% curves from flat geometry. One way to do so is by estimating moduli of annuli.
...
\begin{proof}[Proof of \autoref{thm:rational-approximation}]
  ...
  We then construct simple two-sided curves by concatenating the flow with the arc joining $p_i$ to $p_0$ (see \autoref{fig:coming-from-above}).
  \begin{figure}[h]
    \centering
    \includegraphics{example-image-b}
